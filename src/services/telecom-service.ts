...
export async function getLiveRechargePlans(
  ...
  // We are now only considering plans with the same data amount as 'similar'
  const similarPlans = providerPlans
    .filter(
      (plan) =>
        plan.dailyData === dailyDataUsageGB && plan.validity !== validityDays
    )
    // Add a score based on how close the validity is
    .map((plan) => ({
      ...plan,
      score: Math.abs(plan.validity - validityDays),
    }))
    // Sort by the score, so closer validities are first
